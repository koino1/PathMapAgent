...
# Configuration:
## Installation 
1. Prepare a standard OpenAI API key and export it as an environment variable before running the scripts.
   ```
   export OPENAI_API_KEY=your_openai_api_key
   ```
   Optionally, you can also choose a different model:
   ```
   export OPENAI_MODEL=gpt-4o
   ```
   You can also save the key in a file named `openai_GeneAgent_key` in the project root. The scripts will load it automatically if `OPENAI_API_KEY` is not set.
...
1. Go to the created directory of GeneAgent
   ...
   cd {directory}
   ```
 2. Export your OpenAI API key in the shell where you run the project.
	```
 	export OPENAI_API_KEY=YOUR_OWN_OPENAI_KEY
 	```
    Or create a file named `openai_GeneAgent_key` in the project root and put the key string in that file.
  >[!TIP]
   >If you need to run the variants of GeneAgent, such as chain-of-thought **main_CoT.py** and the summarization for multiple biological terms **main_summary.py**, they will read the same environment variables automatically.
